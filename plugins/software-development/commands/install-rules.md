---
description: Copy this plugin's software development rules into the project's or user's .claude/rules/ so they auto-load by file path
argument-hint: "[project|user]"
---

Install (or update) the software development rules from this plugin as Claude Code rules.
...
- `project` → target `.claude/rules/software-development/` in the current project root. Rules apply only to this project; the project commits them so the whole team gets them.
- `user` → target `~/.claude/rules/software-development/`. Rules apply to every project you work in on this machine; nothing to commit.
...
1. Locate the plugin's `rules/` directory at `${CLAUDE_PLUGIN_ROOT}/rules/`. If that variable was not substituted (it appears literally above), find the newest installed version under `~/.claude/plugins/cache/crafter-ai/software-development/*/rules/`.
2. Resolve the target directory per the scope picked above (project root's `.claude/rules/software-development/`, or `~/.claude/rules/software-development/`).
...
6. If scope was `project`: remind the user to commit `.claude/rules/software-development/` so the whole team gets the rules. If scope was `user`: note that no commit is needed since it applies machine-wide. Either way, mention that re-running this command after a plugin update pulls in rule changes.
